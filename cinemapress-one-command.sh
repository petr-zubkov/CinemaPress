...
# Функция клонирования CinemaPress
clone_cinemapress() {
    log_step "Cloning CinemaPress..."
    
    # Попытка клонировать из официального репозитория
    if git clone https://github.com/CinemaPress/CinemaPress.git temp 2>/dev/null; then
        # Копирование нужных файлов
        cp -r temp/* . 2>/dev/null || true
        cp -r temp/.* . 2>/dev/null || true
        rm -rf temp
        log_success "CinemaPress cloned from official repository"
    else
...
create_basic_structure() {
    log_step "Creating basic CinemaPress structure..."
    
    # Создание основных файлов
    cat > package.json << EOF
{
  "name": "cinemapress-$DOMAIN",
  "version": "1.0.0",
  "description": "CinemaPress website for $DOMAIN",
  "main": "app.js",
  "scripts": {
    "start": "node app.js",
    "dev": "nodemon app.js"
...
// Database connection
mongoose.connect(process.env.MONGODB_URI || 'mongodb://database:27017/cinemapress', {
...
MONGODB_URI=mongodb://database:27017/cinemapress
...
services:
  ...
  database:
    ...
    restart: unless-stopped
    environment:
      - MONGO_INITDB_DATABASE=cinemapress
    volumes:
      - ./data/mongo:/data/db
...
<head>
    ...
    <meta name="description" content="CinemaPress - Your movie streaming platform">
    <style>
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>💻 Powered by CinemaPress | 🎬 Professional Movie Platform</p>
...
setup_autostart() {
    ...
    cat > /usr/local/bin/cinemapress-autostart << EOF
#!/bin/bash
# CinemaPress autostart script

if [ -d "/home/$DOMAIN" ]; then
    cd "/home/$DOMAIN"
    sudo docker-compose up -d
    echo "CinemaPress autostart completed at \$(date)" >> "/home/$DOMAIN/logs/autostart.log"
fi
EOF
    
    sudo chmod +x /usr/local/bin/cinemapress-autostart
    
    # Добавление в crontab
    (crontab -l 2>/dev/null; echo "@reboot /usr/local/bin/cinemapress-autostart") | crontab -
...
create_management_script() {
    ...
    cat > manage.sh << EOF
#!/bin/bash
# CinemaPress Management Script for $DOMAIN
...
case "\${1:-}" in
    ...
    "reset")
        ...
        ;;
    *)
        echo "CinemaPress Management Script"
        echo ""
        echo "Usage: \$0 {start|stop|restart|logs|status|update|backup|restore|reset}"
        echo ""
        echo "Commands:"
        echo "  start    - Start CinemaPress services"
        echo "  stop     - Stop CinemaPress services"
        echo "  restart  - Restart CinemaPress services"
        echo "  logs     - Show service logs"
        echo "  status   - Show service status"
        echo "  update   - Update CinemaPress"
...
show_results() {
    ...
    echo ""
    echo "${GREEN}🎉 CinemaPress Installation Complete!${NC}"
    echo ""
    echo "${CYAN}===========================================${NC}"
    echo ""
...
main() {
    ...
    echo "${CYAN}CinemaPress One-Command Installer${NC}"
    echo "${CYAN}===========================================${NC}"
    echo ""
